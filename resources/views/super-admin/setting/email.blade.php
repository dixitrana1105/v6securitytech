...
@endif
    <div
        ...
        <div class="panel shadow-lg rounded-lg p-8 max-w-6xl mx-auto"
            ...
            <form action="{{ route('email.setup.store') }}" method="POST">
                @csrf
                ...
                @foreach ($setups as $setup)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        {{-- <div>
                            ...
                            <input id="testemail" name="mail_test_email" type="text" placeholder="Test Email" class="form-input" value="{{ old('mail_test_email.' . $loop->index, $setup->mail_test_email ?? '') }}" required />
                        </div> --}}
                                ...
                                <div class="fixed inset-0 bg-[black]/60 z-[999] hidden" :class="open && '!block'">
                                    <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                        <div x-show="open" x-transition x-transition.duration.300
                                            class="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-4">
                                            <div class="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-4 py-2">
                                                <h5 class="font-bold text-lg">Test Email</h5>
                                                <button type="button" class="text-white-dark hover:text-dark" @click="toggle">
                                                    <svg style="max-height: 20px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                            ...
                                            <div class="p-4">
                                                @include('super-admin.setting.email-test')
                                            </div>
                                        </div>
                                    </div>
                                </div>
