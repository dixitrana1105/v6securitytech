    ...
    <div
        ...
        <div class="panel shadow-lg rounded-lg p-8 max-w-6xl mx-auto "
            ...
            @if ($isBlocked)
                ...
                @if ($get_all_data->tenant_block == 1)
                <button
                    class="btn-unblock btn-primary text-white font-bold py-1 px-4 rounded shadow-md transition duration-300"
                    data-visitor-id="{{ $get_all_data->visitor_id }}">
                    UnBlock
                </button>
            @else
            UnBlock
...
<script>
    $(document).on('click', '.btn-unblock', function () {
        let button = $(this);
        let visitorId = button.data('visitor-id');

        $.ajax({
            url: '{{ route("unblock-security") }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
            ...
            error: function (xhr) {
                alert('Error unblocking visitor');
            }
        });
    });
</script>
