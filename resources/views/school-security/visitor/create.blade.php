            ...
            <form action="{{route('school.security.visitor.store')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <div id="imagePreview" class="flex justify-center mb-4 {{ isset($idProofPath) ? '' : 'hidden' }}">
                            <img id="capturedPreview" src="{{ asset($idProofPath ?? 'default/image/path.jpg') }}" alt="Captured Image" class="rounded-lg"
                                style="width: 640px; height: 383px;" />
                        </div>
        ...
        fetch(`/get-students?section=${sectionValue}`)
            ...
            .then(data => {
                data.forEach(function (student) {
                    const option = document.createElement('option');
                    option.value = `${student.name}---${student.reader_id}`;  // Use the correct student identifier
