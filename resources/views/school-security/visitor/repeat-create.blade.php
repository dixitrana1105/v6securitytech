    ...
    <div
        ...
        <div class="panel shadow-lg rounded-lg p-8 max-w-6xl mx-auto "
           ...
           @if ($isBlocked)
            ...
            @if ($unblockVisitors->visitor_block == 1)
            <button
                class="btn-unblock-sch btn-primary text-white font-bold py-1 px-4 rounded shadow-md transition duration-300"
               data-visitor-id="{{$unblockVisitors->visitor_id_detected}}">
                UnBlock
            </button>
        @else
        UnBlock
...
<script>
    $(document).on('click', '.btn-unblock-sch', function () {
        let button = $(this);
        let visitorId = button.data('visitor-id');
        $.ajax({
            url: '{{ route("school.security.visitor.unblock") }}',
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
